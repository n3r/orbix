...
function seasonLabel(s: SeasonSummary): string {
  ...
  return s.name && !/^season\s/i.test(s.name) ? s.name : `Season ${s.seasonNumber}`;
}
  ...
  useEffect(() => {
    ...
    if (first?.fileId) {
      ...
      onPlayEpisode({
        fileId: first.fileId,
        episodeId: first.id,
        title: first.title ?? `Episode ${first.episodeNumber}`,
      });
    }
  }, [playFirstToken, episodes, onPlayEpisode]);
  ...
  return (
    <section className="flex w-full flex-col gap-4 px-6 py-6 md:px-12 lg:px-16">
      <div className="flex items-center justify-between gap-4">
        <h2 className="text-xl font-semibold text-[var(--text)]">Episodes</h2>
        {ordered.length > 1 && (
          <select
            value={selected}
      ...
      <ol className="flex flex-col divide-y divide-[var(--surface-2)]">
        {episodes.map((ep) => {
          const runtime = formatRuntime(ep.runtimeSec);
          const pct =
          ...
          return (
            <li key={ep.id} className="flex gap-4 py-4">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1">
                ...
                {!playable && (
                  <span className="text-xs text-[var(--text-dim)] italic">Not in library</span>
                )}
              </div>
            </li>
          );
        ...
        {episodes.length === 0 && (
          <li className="py-4 text-sm text-[var(--text-dim)]">No episodes found for this season.</li>
        )}
      </ol>
    </section>
  );
