...
- Playback is capability-negotiated: clients POST their profile (containers/codecs/channels) to `/api/playback/info` and get an explicit `direct|remux|transcode` decision + `playSessionId`; HLS routes require `?playSessionId=` (per-viewing ffmpeg isolation in `apps/api/src/playback/{registry,session}.ts`), progress PUTs double as heartbeats, `POST /api/playback/:id/stop` tears down eagerly. The pure decision lives in `packages/core/src/playback/strategy.ts` (`decidePlayback`). HLS output is Apple-spec (multivariant CODECS/RESOLUTION/FRAME-RATE, keyframe-accurate EXTINFs via MediaFile.keyframes + the keyframes queue job, WebVTT subtitle renditions); scripts/verify-hls.mjs is the conformance gate.
