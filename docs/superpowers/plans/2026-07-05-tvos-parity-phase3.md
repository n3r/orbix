...
**Files:**
- Modify: `clients/tvos/Sources/Orbix/Search/SearchView.swift` (fold-in polish: show the results header on the no-results state)
...
- [ ] **Step 4: Wishlist round-trip check (conditional)** — if the **Phase 2 wishlist device-profile server fix has been deployed to the NAS**, verify the title-page wishlist toggle flips optimistically, persists, and is reflected on return to My List. **If the NAS is still on the pre-fix build**, the toggle will optimistically flip then revert on the `400 no_profile` response — record this as the known **stale-NAS wishlist constraint carried over from Phase 2** (not a Phase-3 regression) rather than treating it as a bug. Note which case applied.
